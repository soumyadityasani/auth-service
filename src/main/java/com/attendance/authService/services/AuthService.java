...
//    }
    ...
    public ResponseEntity<ApiResponseDto<?>> assignHod(AssignHodOrCoordinatorRequest request) {
        ...
        if(existingHod.isPresent()) {
            return ResponseEntity.status(HttpStatus.CONFLICT).body(
                    new ApiResponseDto<>(
                            false,
                            "ACTIVE HOD ALREADY EXIST, DEACTIVATE FIRST",
                            null,
                            LocalDateTime.now()
                    )
            );
        ...
        User user= userRepo.findByUserId(request.getUserId()).orElseThrow(()-> new UserNotFoundException("S-404"));
    ...
    public ResponseEntity<ApiResponseDto<?>> updateHod(UpdateHodOrCoordinatorRequest request) {
        ...
        Hod Hod=hodRepo.findByHodAndDepartmentAndAcademicYearAndSemester(hod,request.getDepartment(),request.getAcademicYear(),request.getSemester()).orElseThrow(
                ()-> new UserNotFoundException("S-404")
        );

        User newHod= userRepo.findByUserId(request.getNewUserId()).orElseThrow(()-> new UserNotFoundException("S-404"));

        if(!hod.getUserId().equals(newHod.getUserId())){
            Hod.setHod(newHod);
        }
        if(!Objects.equals(request.getDepartment(), request.getNewDepartment())){
            Hod.setDepartment(request.getNewDepartment());
        }
        if(!Objects.equals(request.getAcademicYear(), request.getNewAcademicYear())){
            Hod.setAcademicYear(request.getNewAcademicYear());
        }
        if(!Objects.equals(request.getSemester(), request.getNewSemester())){
            Hod.setSemester(request.getNewSemester());
        }
        if(Hod.isActive() !=request.isNewIsActive()){
            Hod.setActive(request.isNewIsActive());
        }
    ...
    public ResponseEntity<ApiResponseDto<?>> assignCoordinator(AssignHodOrCoordinatorRequest request) {
        ...
        if(existingCoordinator.isPresent()) {
            return ResponseEntity.status(HttpStatus.CONFLICT).body(
                    new ApiResponseDto<>(
                            false,
                            "ACTIVE COORDINATOR ALREADY EXIST, DEACTIVATE FIRST",
                            null,
                            LocalDateTime.now()
                    )
            );
    ...
    public ResponseEntity<ApiResponseDto<?>> updateCoordinator(UpdateHodOrCoordinatorRequest request) {
        ...
        Coordinator Coordinator =coordinatorRepo.findByCoordinatorAndDepartmentAndAcademicYearAndSemester(coordinator,request.getDepartment(),request.getAcademicYear(),request.getSemester()).orElseThrow(
                ()-> new UserNotFoundException("S-404")
        );

        User newCoordinator = userRepo.findByUserId(request.getNewUserId()).orElseThrow(()-> new UserNotFoundException("S-404"));

        if(!coordinator.getUserId().equals(newCoordinator.getUserId())){
            Coordinator.setCoordinator(newCoordinator);
        }
        if(!Objects.equals(request.getDepartment(), request.getNewDepartment())){
            Coordinator.setDepartment(request.getNewDepartment());
        }
        if(!Objects.equals(request.getAcademicYear(), request.getNewAcademicYear())){
            Coordinator.setAcademicYear(request.getNewAcademicYear());
        }
        if(!Objects.equals(request.getSemester(), request.getNewSemester())){
            Coordinator.setSemester(request.getNewSemester());
        }
        if(Coordinator.isActive() !=request.isNewIsActive()){
            Coordinator.setActive(request.isNewIsActive());
        }
    ...
    @Transactional
    public ResponseEntity<ApiResponseDto<GetHodOrCoordinatorResponse>> getHodAndCoordinator(GetHodOrCoordinatorRequest request) {
        Optional<Hod> hod= hodRepo.findByDepartmentAndAcademicYearAndSemesterAndActiveTrue(request.getDepartment(),request.getAcademicYear(),request.getSemester());

        Optional<Coordinator> coordinator= coordinatorRepo.findByDepartmentAndAcademicYearAndSemesterAndActiveTrue(request.getDepartment(),request.getAcademicYear(),request.getSemester());

        if(hod.isEmpty() && coordinator.isEmpty()){
            return ResponseEntity.ok().body(
                    new ApiResponseDto<>(
                            true,
                            "BOTH NOT ASSIGNED",
        ...
        String hodId= hod.map(value -> value.getHod().getUserId()).orElse(null);
        String coordinatorId= coordinator.map(value -> value.getCoordinator().getUserId()).orElse(null);

        GetHodOrCoordinatorResponse response=GetHodOrCoordinatorResponse.builder()
                .hodId(hodId)
                .coordinatorId(coordinatorId)
                .build();

        return ResponseEntity.ok().body(
                new ApiResponseDto<GetHodOrCoordinatorResponse>(
                        true,
                        "SUCCESS",
                        response,
                        LocalDateTime.now()
